...
__all__ = ["NestedGNNExplainer"]


class NestedGNNExplainer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super(NestedGNNExplainer, self).__init__()
        self.model = model
    ...
    def _loss_regularize(self, loss, out_feat_mask=None, out_edge_mask=None, in_feat_mask=None, in_edge_mask=None):
        ...
        # CALCULATE LOSS FOR EDGE MASKS 
        
        # OUTER GRAPH
        if out_edge_mask is not None:      
            out_edge_mask = out_edge_mask.sigmoid()
            # Edge mask sparsity regularization
            loss = loss + self.alpha1 * torch.sum(out_edge_mask)
            # Edge mask entropy regularization
            ent = -out_edge_mask * torch.log(out_edge_mask + eps) - (
                1 - out_edge_mask
            ) * torch.log(1 - out_edge_mask + eps)
            loss = loss + self.alpha2 * ent.mean()
        
        # INNER GRAPH
        if in_edge_mask is not None:
            in_edge_mask = in_edge_mask.sigmoid()
            # Edge mask sparsity regularization
            loss = loss + self.alpha1 * torch.sum(in_edge_mask)
            # Edge mask entropy regularization
            ent = -in_edge_mask * torch.log(in_edge_mask + eps) - (
                1 - in_edge_mask
            ) * torch.log(1 - in_edge_mask + eps)
            loss = loss + self.alpha2 * ent.mean()

        # CALCULATE LOSS FOR FEATURE MASKS
        
        # OUTER GRAPH
        if out_feat_mask is not None:
            out_feat_mask = out_feat_mask.sigmoid()
            # Feature mask sparsity regularization
            loss = loss + self.beta1 * torch.mean(out_feat_mask)
            # Feature mask entropy regularization
            ent = -out_feat_mask * torch.log(out_feat_mask + eps) - (
                1 - out_feat_mask
            ) * torch.log(1 - out_feat_mask + eps)
            loss = loss + self.beta2 * ent.mean()
        
        # INNER GRAPH
        if in_feat_mask is not None:
            in_feat_mask = in_feat_mask.sigmoid()
            # Feature mask sparsity regularization
            loss = loss + self.beta1 * torch.mean(in_feat_mask)
            # Feature mask entropy regularization
            ent = -in_feat_mask * torch.log(in_feat_mask + eps) - (
                1 - in_feat_mask
            ) * torch.log(1 - in_feat_mask + eps)
            loss = loss + self.beta2 * ent.mean()

        return loss

    def explain_node(self, node_id, inner_g, outer_g, inner_feat, outer_feat, **kwargs):
        ...
        self.model.eval()
        num_nodes = outer_g.num_nodes()
        num_edges = outer_g.num_edges()

        # Extract node-centered k-hop subgraph and
        # its associated node and edge features.
        sg, inverse_indices = khop_in_subgraph(outer_g, node_id, self.num_hops)
        sg_nodes = sg.ndata[NID].long()
        sg_edges = sg.edata[EID].long()
        feat = outer_feat[sg_nodes]
        ...
        with torch.no_grad():
            ...
            logits = self.model(inner_g, outer_g, inner_feat, outer_feat)
            pred_label = logits.argmax(dim=-1)

        out_feat_mask, out_edge_mask = self._init_masks(sg, feat)

        params = [out_feat_mask, out_edge_mask]
        ...
        for _ in range(self.num_epochs):
            ...
            h_out = outer_feat * out_feat_mask.sigmoid()
            
            #logits = self.model(
            #    graph=sg, feat=h, eweight=edge_mask.sigmoid(), **kwargs
            #)
            logits = self.model(
                inner_g, sg, inner_feat, h_out, outer_edge_weight=out_edge_mask.sigmoid(),**kwargs
            )
            ...
            loss = self._loss_regularize(loss, out_feat_mask, out_edge_mask)
        ...
        out_feat_mask = out_feat_mask.detach().sigmoid().squeeze()
        out_edge_mask = out_edge_mask.detach().sigmoid()

        return inverse_indices, sg, out_feat_mask, out_edge_mask

    def explain_graph(self, inner_g, outer_g, inner_feat, outer_feat, **kwargs):
        ...
        in_feat_mask, in_edge_mask = self._init_masks(inner_g, inner_feat)
        out_feat_mask, out_edge_mask = self._init_masks(outer_g, outer_feat)

        params = [in_feat_mask, in_edge_mask, out_feat_mask, out_edge_mask]
        optimizer = torch.optim.Adam(params, lr=self.lr)
        ...
        for _ in range(self.num_epochs):
            optimizer.zero_grad()
            h_in = inner_feat * in_feat_mask.sigmoid()
            h_out = outer_feat * out_feat_mask.sigmoid()
            
            logits = self.model(
                inner_g, outer_g, h_in, h_out, outer_edge_weight=out_edge_mask.sigmoid(), inner_edge_weight =in_edge_mask.sigmoid(),**kwargs
            )
            
            log_probs = logits.log_softmax(dim=-1)
            loss = -log_probs[0, pred_label[0]]
            loss = self._loss_regularize(loss, out_feat_mask, out_edge_mask, in_feat_mask, in_edge_mask)
            loss.backward()
            optimizer.step()
        ...
        in_feat_mask = in_feat_mask.detach().sigmoid().squeeze()
        in_edge_mask = in_edge_mask.detach().sigmoid()

        return out_feat_mask, out_edge_mask, in_feat_mask, in_edge_mask
